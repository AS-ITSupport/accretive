...
function phptemplate_breadcrumb($breadcrumb) {
  ...
  if (!empty($breadcrumb)) {
    ...
    if ($num_uri_items>2&&$cur_page[1]=='news-and-events'||$num_uri_items>2&&$cur_page[1]=='as-involved') {

       $breadcrumb[1] = ($cur_page[1]=='news-and-events'?'<a href="/news-and-events"> News and Events </a>':'<a href="/as-involved"> AS Involved </a>');       

       $breadcrumb[] = (strlen(drupal_get_title())>50?substr(drupal_get_title(),0,49).'...':substr(drupal_get_title(),0,49));

    }    

    //check for event registration page	

	if ($num_uri_items==2&&$cur_page[1]=='event-registration') {

	  $breadcrumb[1] = '<a href="/news-and-events"> News and Events </a>';

	  $breadcrumb[] = (strlen($_SESSION['event_title'])>50?substr($_SESSION['event_title'],0,49).'...':substr($_SESSION['event_title'],0,49));
...
function phptemplate_preprocess_page(&$vars) {
  ...
  $left2col = array('about-as/management-team', 'offices', 'job-application-0', 

                    'consulting-services/newsletters', 'careers/immediate-openings');
  ...
  if (count($temp)==2&&$temp[0]=='consulting-solutions'||$temp[0]=='executive-search'&&count($temp)==1) {
    ...
    if (!empty($engagement_data)||$temp[0]=='executive-search') {
      ...
      $vars['right'] .= '<a href="/'.$engagement_path.'">Click here to view examples of engagements that we have performed for our clients.</a>';        
  ...
  if ($alias=='event-registration') {
    ...
    $vars['title'] = $_SESSION['event_title'];
  ...
  foreach ($menu as $menu_item) {   
    ...
    if ($menu_item['link']['hidden'] == 0) {
      ...
      if(strpos($menu_alias, "/") == 0) {
        ...
        //print_r($menu_item['link']['link_title']);
      ...
      //node_load(theme_get_setting('headquarters_office'));
